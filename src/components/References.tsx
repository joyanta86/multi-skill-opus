...
export const References = () => {
  ...
  const references = [
    {
      name: 'Samu Penttilä',
      title: 'Team Leader Kokkola – IT Services | System Specialist',
      company: 'JNT Oy',
                ...
                <Card className="p-4 sm:p-5 bg-card border-border hover:border-primary/30 transition-all duration-500 ease-out group h-full hover:shadow-[var(--glow-primary)] hover:-translate-y-1">
                  <div className="flex items-start gap-3 mb-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="text-sm sm:text-base font-semibold text-foreground mb-0.5 tracking-tight truncate">{reference.name}</h3>
                      <p className="text-xs text-primary/80 font-medium leading-tight">{reference.title}</p>
                      <p className="text-xs text-muted-foreground font-mono">{reference.company}</p>
                    </div>
                  </div>

                  <p className="text-muted-foreground mb-4 text-xs leading-relaxed">{reference.description}</p>

                  <div className="space-y-1.5 mb-4">
                    <a href={`mailto:${reference.email}`}
                      className="flex items-center gap-2 text-xs text-muted-foreground hover:text-primary transition-colors duration-300 font-mono truncate">
                      <Mail className="h-3.5 w-3.5 flex-shrink-0" />
                      <span className="truncate">{reference.email}</span>
                    </a>
                    <a href={`tel:${reference.phone.replace(/\s/g, '')}`}
                      className="flex items-center gap-2 text-xs text-muted-foreground hover:text-primary transition-colors duration-300 font-mono">
                  ...
                  <div className="flex gap-2">
                    {reference.linkedin && (
                      <Button variant="outline" size="sm" className="flex-1 gap-1.5 text-xs border-border hover:border-primary/40 hover:bg-primary/5 transition-all duration-300" asChild>
